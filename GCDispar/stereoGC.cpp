...
// funtion to give a node id given i, j, and k (to make indexes comprehension easier)
int NodeId(int i, int nx, int j, int ny, int k){
...
int main() {
    ...
    Graph<int,int,int> G(nx*ny*nd, 6*nx*ny*nd); // at most, each node has 6 edges (4 neighbours, plus sinks and sources)
    G.add_node(nx*ny*nd);
    for (int i=0; i<nx; i++){
	for (int j=0; j<ny; j++){
	    for (int k=0; k<nd; k++){
		int x = n + zoom*i; int y = n + zoom*j; // position in initial image
		int d = dmin + k; // current disparity
		int node_id = NodeId(i, nx, j, ny, k); // as advised

		float wpj = INF;
		if (x + d > n || x + d > w2 -n){ // P in I2M
		    float Dpd = wcc*rho(zncc(I1,I1M,I2,I2M,x,y,x+d,y,n));
		    float Kp = 1;
		    int neighbours_in_image = 0;
		    ...
		    Kp += neighbours_in_image*(nd-1)*lambda;
		    wpj = Dpd + Kp; // slide 89
		}
		...
		if (k == 0){ // source nodes
		    ...
		    int prev_node_id = NodeId(i, nx, j, ny, k-1);
		    G.add_edge(prev_node_id, node_id, min((float)INF, wpj), 0);
		    if (k == nd-1){ // sink nodes
		    	G.add_tweights(node_id, 0, INF);
		    }
		}
                if(i+1<nx){ // right neighbour edge
                    int right_node_id = NodeId(i+1,nx,j,ny,k);
                    G.add_edge(node_id,right_node_id,lambda,lambda);
                }
                if(j+1<ny){ // down neighbour edge
                    int down_node_id = NodeId(i,nx,j+1,ny,k);
    ...
    for (int i=0;i<nx;i++) {
	    ...
	    int last_layer_node_id = NodeId(i, nx, j, ny, nd-1);
	    if (G.what_segment(first_layer_node_id) == Graph<int,int,int>::SINK){
		D(i,j) = dmin;
	    } else if (G.what_segment(last_layer_node_id) == Graph<int,int,int>::SOURCE){
		D(i,j) = dmax;
	    } else {
		int current_node_id = NodeId(i, nx, j, ny, 0);
		for (int d=1; d<nd; d++){
		    int prev_node_id = current_node_id;
		    current_node_id = NodeId(i, nx, j ,ny, d);
